...
- **Real two-compression evaluation run: still blocked**, same root cause as checkpoint C-D. Progress since C-D's own note: the local proxy-port problem is now genuinely fixed (see below), narrowing the remaining blocker to provider-side alone.
- **Simple/Operator simplification: correctly still not attempted** — it is the one item the doc's own wording actually gates on evidence from this run, and that run hasn't happened.

**Local environment fix (2026-08-20, found while retrying checkpoint C-D):** the real root cause behind the whole "stale proxy port" saga across sessions was not `.env` drift alone — `src/thesisound/http_proxy.py`'s `DEFAULT_HTTP_PROXY = "http://127.0.0.1:10809"` is a hardcoded Python-level fallback used whenever `THESISOUND_HTTP_PROXY` is absent from the environment, so commenting the `.env` line out (tried first, on the belief the setting was mistakenly copied from a server `.env`) still fell through to the same dead port. This codebase's Gemini API-key path also deliberately refuses to run unproxied (`ModelConfigurationError` on `THESISOUND_HTTP_PROXY=none`) — proxying is required by design here, not optional. Fixed by finding the actual live local Xray port with `netstat -ano | grep "127.0.0.1:108"` (10808, matching the shell's own `$HTTP_PROXY`) and setting it in `.env`; confirmed via the ledger that the `ConnectError`/`ModelConfigurationError` class fully disappeared on the next attempt. Full history in agent memory (`okian-proxy-tunnel-outage-2026-08-20`, `local-live-run-blockers`).

With the proxy fixed, checkpoint C-D's resumed run reached the script-writing stage cleanly but could not complete it: Gemini's key pool re-exhausted after a handful of retries within this session (tight retry loops don't give a reset pool time to recover), and Okian failover returned three *different* error codes (500, 400, 406) across near-identical probes within a few minutes — live evidence of unstable backend infrastructure on Okian's side, not a request-shape bug here. The project (`3dfd0711-eccc-4526-a635-6aaf9da7470c`) remains the resume point for both checkpoint C-D's remaining script/audio verification and step 25's two-compression evaluation once either provider has had real recovery time — retry with real idle time between Gemini attempts, and re-probe Okian directly before resuming, rather than tight-looping.
