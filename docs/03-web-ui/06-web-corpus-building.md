# 06 — ساخت واقعی مجموعه شواهد از رابط وب

as-built؛ ادامهٔ مستقیم [`05-web-ui-auth-and-first-slice.md`](05-web-ui-auth-and-first-slice.md).
...
- `plan_evidence_extraction` — با brief فعلی و همان seedهای طرح واقعی (`resolve_extraction_seeds` روی نقشهٔ مؤثر: cache ⊕ overlay) — دقیقاً همان `profile` و همان `selected_block_ids` ذخیره‌شده را بدهد.
...
این سخت‌گیری فقط برای درس پرسش‌محور معنا دارد. در درس منبع‌محور (`lesson_intent == source_coverage`) بلوک‌ها با رتبه‌بندی مدت انتخاب نمی‌شوند، با سلول‌های درون‌دامنه انتخاب می‌شوند و عمق روی `extended` قفل است؛ پس بازبرنامه‌ریزی هم باید با همان سلول‌ها انجام شود. بدون seed، طرح بازساخته هرگز با طرح ذخیره‌شده یکی نمی‌شود و منبع تمام‌شده در هر `confirm` از صفر ساخته می‌شود. به همین دلیل تغییر مدت در این حالت استخراج را دوباره نمی‌خرد و فقط episodeها را دوباره بسته‌بندی می‌کند — `episode_duration_cost` هم با همان seedها پیش‌بینی می‌کند تا با اجرای واقعی اختلاف پیدا نکند.
