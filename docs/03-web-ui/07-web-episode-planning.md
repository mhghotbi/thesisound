# 07 — ارزیابی پوشش و طرح اپیزود در رابط وب

as-built؛ ادامهٔ مستقیم [`06-web-corpus-building.md`](06-web-corpus-building.md).
...
- تنظیم مدت (در محدودهٔ قابل‌پشتیبانی) و اجرای attempt جدید؛
- افزودن منبع؛
- تغییر تمرکز Research Brief.

مدت تازه تا سقف قابل‌پشتیبانی پذیرفته می‌شود.

## استفادهٔ دوباره از مرحله‌های مدلی

هر attempt تازه — retry، تغییر مدت یا اجرای دوباره روی همان corpus — نباید کار مدلی تمام‌شده را دوباره بخرد. دو مرحله مدلی هستند: `coverage audit` و `episode plan`. بقیهٔ مرحله‌ها deterministic و ارزان‌اند و همیشه دوباره محاسبه می‌شوند تا با ورودی فعلی سازگار بمانند.

کلید ورودی هر مرحله در فایل زیر ذخیره می‌شود:

```text
<workspace>/<project_id>/episode/stage-inputs.json
```

کلید از این‌ها ساخته می‌شود: فهرست منابع تأییدشده، شناسهٔ مدعاهای corpus، planهای استخراج شاهد، و Research Brief.

تفاوت دو مرحله در مدت است:

- کلید `coverage` بدون `target_duration_minutes` ساخته می‌شود. سنجش کفایت منابع می‌گوید این corpus حداکثر چند دقیقه را پشتیبانی می‌کند؛ این پاسخ به مدت درخواستی وابسته نیست. هنگام استفادهٔ دوباره فقط `can_plan_episode` برای مدت فعلی دوباره محاسبه می‌شود.
- کلید `plan` با `target_duration_minutes` ساخته می‌شود. طرح گفتار خودش بر اساس مدت شکل می‌گیرد، پس مدت تازه یعنی طرح تازه.

نتیجهٔ عملی:

- retry پس از شکست ساخت طرح: سنجش کفایت منابع دوباره اجرا نمی‌شود؛
- تغییر مدت پس از blocked: سنجش کفایت منابع دوباره اجرا نمی‌شود، فقط طرح دوباره ساخته می‌شود؛
- تغییر corpus یا Brief: هر دو مرحله از نو اجرا می‌شوند؛
- هر اجرای تازهٔ coverage، کلید `plan` را باطل می‌کند، چون طرح قبلی روی پاسخ قبلی ساخته شده بود.

`model_run_ids` در manifest فقط به اجرای مدلی واقعی اشاره می‌کند؛ استفادهٔ دوباره run تازه‌ای نمی‌سازد.
...
- attempt تازه روی همان corpus، مرحله‌های مدلی تمام‌شده را دوباره اجرا نکند؛
