...
هر attempt تازه — retry، کاهش مدت یا اجرای دوباره روی همان corpus — نباید کار مدلی تمام‌شده را دوباره بخرد. دو مرحله مدلی هستند: `coverage audit` و `episode plan`. بقیهٔ مرحله‌ها deterministic و ارزان‌اند و همیشه دوباره محاسبه می‌شوند تا با ورودی فعلی سازگار بمانند.
...
- کاهش مدت پس از blocked: سنجش کفایت منابع دوباره اجرا نمی‌شود، فقط طرح دوباره ساخته می‌شود؛
