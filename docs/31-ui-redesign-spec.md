# 30 — Spec بازطراحی UI رابط وب Thesisound

تاریخ: ۲۰۲۶-۰۸-۰۹ · پیش‌نیاز: `docs/29-ui-ux-audit.md` · مرجع بصری: صفحه‌های بازطراحی‌شده در پروژه طراحی («Thesisound Flow»، ۱۰ صفحه مسیر کامل)
...
- **حالت پیشرفته (Operator):** toggle صریح در header، ذخیره در session، برچسب دائمی «حالت پیشرفته» هنگام فعال بودن. Simple پیش‌فرض است. یک backend، یک state machine؛ تفاوت فقط در density، واژگان و کنترل‌های اضافه.

## ۲. مدل نمایش state (سه محور، طبق docs/17)
...
- واژه‌نامه ثابت حالت ساده: brief→«برداشت»، corpus→«مجموعه منابع»، evidence→«شواهد»، claim→«ادعای مستند»، script→«متن اپیزود»، verify→«راستی‌آزمایی»، archive→«بایگانی»، parse→«استخراج متن». واژه انگلیسی در حالت ساده فقط برای نام فایل/URL داخل `<bdi dir="ltr">`.
