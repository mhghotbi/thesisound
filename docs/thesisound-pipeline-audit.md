...
### بودجه‌ی تحلیل خروجی‌آگاه — R5 برطرف شد

| کنترل | هدف | پیش از R5 | پس از R5 | نتیجه |
|---|---:|---:|---:|---|
| `evidence_input_token_budget` | 18,000 | **55,913** | **18,307** | یک overshoot تک-block مجاز است؛ `18,307 - 1,614 = 16,693 ≤ 18,000` |
| `block_coverage_target` | 0.35 | **0.2166** | **0.0709** | budget اکنون binding است؛ target پوشش در این budget قابل‌دستیابی نیست |

علت پیشین در `analysis_profile.py:149-179` بود: مجموعه‌ی `selected` قبل از هر بررسی بودجه با اولین block هر section required seed می‌شد؛ بنابراین حلقه‌ی رتبه‌بندی از همان ابتدا متوقف می‌شد.

R5 seedها را رتبه‌بندی و به ۶۰٪ `target_tokens` محدود می‌کند و باقی budget را به ranked fill می‌سپارد. بازپخش artifact واقعی: **۴۰ → ۱۳ block** و **۵۵٬۹۱۳ → ۱۸٬۳۰۷ token**. پروژه‌های تحلیل‌شده پیش از R5 در `corpus_reuse` یک‌بار cache miss خواهند داشت و evidence را دوباره استخراج می‌کنند؛ این بازتحلیل درست است، نه regression کش.

**اثر جانبی روی گیت `evidence-retention`.** آستانه‌ی ۸۵٪ کسری از توکن‌های *planned* است، پس کوچک‌شدن plan آن را حساس‌تر می‌کند: هر block در plan ۱۳تایی ۶.۹–۸.۸٪ جرم است، در برابر ۲–۳٪ قبلی. اندازه‌گیری‌شده روی همان plan — ۱ رد: ۹۱–۹۳٪ (pass)؛ ۲ رد: ۸۲.۵–۸۶.۲٪ (مرزی)؛ ۳ رد: ≤۷۹٪ (شکست سخت). اجرای واقعی ۴ از ۴۰ بلوک (۱۰٪) را رد کرد و با ۹۲.۰۵٪ عبور کرد. اصلاح: بزرگ‌ترین تلفات تکی در برابر قاعده‌ی ۸۵٪ بخشیده می‌شود اما retention کل هرگز نباید زیر **۷۵٪** برود (`evidence_retention_holds`). نتیجه روی plan ۱۳تایی: ۲ رد عبور می‌کند، ۳ رد همچنان مسدود است. آستانه‌ی ۰.۸۵ دست‌نخورده ماند.

**چیزی که هنوز اندازه‌گیری نشده.** claim yield با ۱۳ بلوک به‌جای ۴۰ حدود یک‌سوم می‌شود (۳۶ بلوک نگه‌داشته‌شده ۷۰ claim داد؛ ۱.۹۴ به‌ازای بلوک ⇒ تخمین ≈۲۵). گیت G6 (`can_plan_episode`) روی ledger قضاوت مدل است، نه روی plan؛ پس تا اجرای واقعی بعدی نمی‌دانیم `recommendation` و `max_supported_minutes` تغییر می‌کنند یا نه. رویداد `corpus.evidence_yield` اکنون `claims_per_kept_block` را ثبت می‌کند تا این عدد بین اجراها قابل پیگیری باشد. **این تنها ریسک باز R5 است.**
