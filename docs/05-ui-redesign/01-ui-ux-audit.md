# 01 — ممیزی UI/UX رابط وب Thesisound

تاریخ: ۲۰۲۶-۰۸-۰۸ · مبنا: کد فعلی `src/thesisound/web/` روی branch `main` · اسناد مرجع: PRODUCT.md، DESIGN.md، [`../03-web-ui/`](../03-web-ui/) (اسناد ۰۱–۰۴)
...
1. **ناوبری دوگانه و متناقض.** هر صفحه هم یک `workflow-switcher` سراسری دارد و هم یک `step-rail` دست‌نویس که تعداد و نام مراحلش صفحه‌به‌صفحه عوض می‌شود (۴ مرحله → ۵ مرحله با نام‌های متفاوت). کاربر هیچ نقشه ثابتی از سفر ندارد و Project Overview (S-03 در `../03-web-ui/03-operator-screen-inventory.md`) اصلاً پیاده نشده است.
2. **حذف منبع، حذف واقعی و بی‌بازگشتِ فایل خام کاربر با یک POST بدون هیچ تأیید است** — در حالی که متن UI در همان صفحه قول می‌دهد «فایل‌های خام شما حذف نمی‌شوند». نقض مستقیم `../03-web-ui/04-error-and-recovery-ux.md` (impact summary) و data-loss prevention.
...
- این مخزن به‌صورت فقط‌خواندنی mount شده؛ تغییر کد مستقیم در repo از این محیط ممکن نیست. اصلاح‌ها به‌صورت spec دقیق ([`02-ui-redesign-spec.md`](02-ui-redesign-spec.md)) و طراحی مرجع تحویل می‌شوند.
...
|---|---|---|---|---|
| Project Overview | `../03-web-ui/03-operator-screen-inventory.md` S-03: نقشه وضعیت + action بعدی در `/projects/{id}` | **وجود ندارد**؛ فهرست پروژه‌ها مستقیم به میانه یک stage لینک می‌دهد | کاربر برگشته هیچ نمای کلی ندارد | **P0** |
| ناوبری پروژه | `../03-web-ui/01-operator-user-workflow.md`: شش تب ثابت، disabled با توضیح | دو سیستم موازی: workflow-switcher + step-rail دست‌نویس ناسازگار (۴ یا ۵ مرحله، نام‌های متفاوت در new/sources/episode/script/audio) | نقشه سفر ناپایدار؛ «تأیید برداشت» در script/audio از rail حذف می‌شود | **P0** |
| سه محور state | `../03-web-ui/02-interface-state-model.md`: lifecycle / execution / attention جدا | `read_models.py` همه را در یک label+tone فشرده می‌کند؛ badge وضعیت run خام انگلیسی کنار عنوان فارسی | passed_with_warnings، stale، interrupted اصلاً قابل‌نمایش نیستند | **P0** |
| مرحله جاری پردازش | progress واقعی | منطق `is-current` در `processing.html` وارونه است (سطر ۲۶) | نمایش state غیرواقعی | **P0** |
| Polling | `../03-web-ui/02-interface-state-model.md`: HTMX partial، حفظ scroll/focus، توقف در idle | `window.location.reload()` هر ۳s در ۴ صفحه؛ HTMX لود شده و استفاده نشده | reset کامل صفحه، شکستن a11y و فرم/disclosure باز | **P0** |
| حذف منبع | اسناد ۰۳ و ۰۴ در `../03-web-ui/`: impact summary اجباری، بدون one-click delete | POST مستقیم + `shutil.rmtree` روی uploads (source_routes.py ~۳۷۸) بدون confirmation؛ دکمه «حذف منبع» هم‌وزن «حذف از مجموعه» کنار هم | حذف بی‌بازگشت فایل خام با یک کلیک اشتباه؛ تناقض با متن UI «فایل‌های خام حذف نمی‌شوند» | **P0** |
...
| Error UX | `../03-web-ui/04-error-and-recovery-ux.md`: ErrorRecord ساختاریافته، anatomy هفت‌بخشی | `last_error` خام (شامل پیام exception/provider) مستقیم render می‌شود؛ `workflow_error` از query param خوانده و نمایش داده می‌شود؛ recovery همیشه فقط «تلاش دوباره» | تفکیک E1–E12، retry budget و strategy جایگزین وجود ندارد | **P1** |
...
| Brief | `../03-web-ui/03-operator-screen-inventory.md` S-04: Save≠Confirm، diff، fieldهای کامل | Save/Confirm جدا ✓؛ اما learning objectives و audience قابل‌ویرایش نیستند و diff وجود ندارد | جزئی، پذیرفتنی برای slice | P2 |
...
| a11y live region | `../03-web-ui/02-interface-state-model.md`: aria-live محدود برای stage فعال | هیچ `aria-live` در کل codebase نیست (grep: صفر نتیجه) | screen reader از reload هم آسیب می‌بیند | P1 |
...
- **Root cause:** الگوی POST-redirect بدون لایه confirmation؛ نبود `ImpactSummary` که `../03-web-ui/04-error-and-recovery-ux.md` الزام کرده.
...
- **Problem:** S-03 (`../03-web-ui/03-operator-screen-inventory.md`) پیاده نشده. کاربری که بعد از یک هفته برمی‌گردد مستقیماً وسط یک فرم یا صفحه وضعیت فرود می‌آید، بدون «چه شد، کجاییم، چه مانده».
...
- **Problem:** anatomy هفت‌بخشی `../03-web-ui/04-error-and-recovery-ux.md` (چه شد/اثر/چرا/پیشنهاد/گزینه‌ها/جزئیات/وضعیت artifact) پیاده نشده؛ کاربر اصلی متن فنی می‌بیند و کاربری که خطایش retryable نیست هم فقط دکمه retry دارد.
...
**اصلاح:** StatusLabel دو-جزئی (وضعیت مرحله + نیاز به اقدام) طبق `../03-web-ui/02-interface-state-model.md`؛ محاسبه attention در server. **پذیرش:** passed_with_warnings و stale نمایش متمایز دارند.
...
`_workflow_navigation.html`: مراحل قفل‌شده `<span aria-disabled="true">` بدون هیچ توضیح precondition (الزام `../03-web-ui/01-operator-user-workflow.md`: «disabled با توضیح»)؛ دکمه تأیید corpus هم `disabled` بدون متن متصل (`aria-describedby`) است.
...
`script.html` کل خروجی را فقط با `script_run.status == "succeeded" and script and checks and verification` نشان می‌دهد؛ اگر run record پس از restart ناقص/غایب باشد ولی project state `SCRIPT_VERIFIED` و artifactها معتبر باشند، صفحه خالی می‌ماند. (**نیازمند اجرای زنده برای تأیید**؛ از کد، مسیر fallback دیده نمی‌شود.) رفتار interrupted/reconcile (اسناد ۰۲ و ۰۴ در `../03-web-ui/`) در هیچ template ای وجود ندارد.
...
- **F-19 microcopy:** «brief»، «corpus»، «artifact»، «archive»، «quality-gate» در متن فارسی Simple Mode؛ واژه‌نامه فارسی ثابت لازم است (واژه‌نامهٔ کامل در [`03-product-language.md`](03-product-language.md)). لحن خوب «من این‌طور فهمیدم» باید معیار بقیه صفحات باشد.
...
- Insufficient corpus بدون «Continue anyway» و با کاهش مدت/افزودن منبع — مطابق `../03-web-ui/04-error-and-recovery-ux.md`.
...
جزئیات طراحی مقصد در [`02-ui-redesign-spec.md`](02-ui-redesign-spec.md).
