# 04 — بودجه تحلیل وابسته به خروجی

مرتبط: این سند لایهٔ سوم [`03-one-source-evidence-pipeline.md`](03-one-source-evidence-pipeline.md) را تفصیل می‌دهد؛ خروجی آن ورودی [`05-episode-preparation.md`](05-episode-preparation.md) است.
...
> **بازنگری ۲۰۲۶-۰۸-۱۹ ([سند ۱۰](../01-foundations/10-personal-learning-companion-development-plan.md) §6):** این سند برای نیت `focused_question` کامل و معتبر است. برای نیت `source_coverage` «خروجی درخواستی» دیگر مدت نیست، بلکه **دامنه × فشردگی** است: عمق همیشه `extended` ولی فقط روی بلاک‌های سلول‌های مفهومیِ در دامنه (استخراج تنبل به‌ازای بخش)؛ `max_claims_per_block = 12` با گذر دوم برای بلاک‌های متراکم؛ مدت خروجی از جمع دقیقهٔ سلول‌ها مشتق می‌شود و به بخش‌ها شکسته می‌شود. جدول profileها و فرمول بودجهٔ زیر برای `source_coverage` به کار نمی‌رود. کران ۵–۱۲۰ در پنج مدل به یک ثابت `MAX_PART_MINUTES` جمع می‌شود.
