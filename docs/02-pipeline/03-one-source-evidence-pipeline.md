# 03 — One-source evidence pipeline
...
در این نسخه، Document Map برای یک فصل یا سند محدود طراحی شده است؛ اگر مجموع متن از budget پیکربندی‌شده بزرگ‌تر باشد، stage متوقف می‌شود. برای جزئیات محدودیت و طرح آینده (نقشهٔ سلسله‌مراتبی book → chapter → section) به بخش «کارهای بعدی» در [`04-output-aware-analysis-budget.md`](04-output-aware-analysis-budget.md) مراجعه کنید.
