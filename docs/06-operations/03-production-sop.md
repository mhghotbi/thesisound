...
| 5 | `evidence-retention` | Machine | Extraction plan and block outcomes | Source-analysis manifest | `src/thesisound/services/source_analysis_service.py:62` | Less than 85% of planned token mass survived extraction, even after forgiving the largest single lost block. | Restore enough planned evidence or reduce the source scope. |
...
| 8 | `script-checks` | Machine | Script, plan, evidence packs and glossary | Script check report | `src/thesisound/services/script_checks.py:96` | A deterministic blocking violation exists. | Correct blocking structure or grounding defects. |
