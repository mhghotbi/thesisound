# 02 — Source Discovery, large-document mapping, and workflow revision

Related: candidate search itself runs through Gemini grounding — see [`01-gemini-grounding.md`](01-gemini-grounding.md) for the per-stage policy and audit artifacts behind step 2 below. This doc covers three distinct topics: the search-to-corpus UX (§1), large-document partitioning (§2), and rewind/revision semantics (§3).
